...
// Minimal-API model binding is case-insensitive, but the manual
// JsonSerializer.Deserialize calls below are not unless told to be.
// Without this, a standard camelCase body like {"title":...} silently
// fails to bind and the handler reports the field as missing.
var JsonOpts = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };

var connString = builder.Configuration.GetConnectionString("Sql")
	?? "Host=localhost;Port=5432;Database=pantryscandb;Username=pantryscan;Password=pantryscan;";
...
app.MapPut("/items/{id:int}", async (int id, ItemUpdateDto dto) =>
{
	...
	await conn.ExecuteAsync(@"
		...
		SET Quantity = COALESCE(@Quantity, Quantity),
		    ...
		    NeedsReview = CASE WHEN @Name IS NOT NULL THEN FALSE ELSE NeedsReview END
		WHERE ItemId = @id",
...
app.MapPost("/recipes", async (HttpContext ctx) =>
{
	...
	// Honour idempotency keys the way every other POST endpoint does. This has to
	// happen here rather than after the format branch, because the key is read
	// straight off the raw JSON before either DTO is deserialised.
	string? idempotencyKey = null;
	foreach (var prop in new[] { "idempotencyKey", "IdempotencyKey" })
	{
		if (root.TryGetProperty(prop, out var ik) && ik.ValueKind == JsonValueKind.String)
		{
			idempotencyKey = ik.GetString();
			break;
		}
	}
	var idem = await CheckIdempotency(conn, idempotencyKey);
	...
	if (isFrontendFormat)
	{
		var dto = JsonSerializer.Deserialize<RecipeCreateFrontendDto>(json, JsonOpts);
	...
	else
	{
		...
		var dto = JsonSerializer.Deserialize<RecipeCreateDto>(json, JsonOpts);
...
static async Task EnsureSchemaAsync(string connString)
{
	...
	// CREATE TABLE IF NOT EXISTS is not atomic against concurrent creation: two
	// sessions can both pass the existence check and then collide, failing with a
	// duplicate key on pg_type/pg_class. An advisory lock serialises schema setup
	// so several instances (or parallel test fixtures) can start simultaneously.
	const long SchemaLockKey = 0x50414E54; // "PANT"
	await conn.ExecuteAsync("SELECT pg_advisory_lock(@key)", new { key = SchemaLockKey });
	try
	{
	await conn.ExecuteAsync(@"
	...
	{
		await conn.ExecuteAsync("SELECT pg_advisory_unlock(@key)", new { key = SchemaLockKey });
	}
}
...
app.MapPost("/shopping/items", async (ShoppingItemDto dto) =>
{
	...
	await conn.ExecuteAsync(@"
		...
		SELECT @ClientId, @Name, @Qty, @Category, @Store, @Note, @RecipesJson
		WHERE NOT EXISTS (SELECT 1 FROM ShoppingItems WHERE ClientId = @ClientId);",
		new
		{
